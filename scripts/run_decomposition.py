...
from condensation_trainer import Condensation_Trainer
...
class ModelArguments:
    ...
    total_frozen_mask_str: str = field(
        default=None,
        metadata={
            "help": (
                "string of layers to be frozen. used only for resuming from checkpoint."
            )
        }
    )
...
class DataTrainingArguments:
    ...
    condense_epoch: int = field(
        default=1,
        metadata={
            "help": (
                "Identify number of epochs spent on each layer for condensation."
            )
        },
    )
    max_budget: int = field(
        ...
        metadata={
            "help": (
                "Maximum number of items with blockwise granularity to be removed."
            )
        },
    )
    ...
    sparsity_ratio: float = field(
        ...
        metadata={
            "help": (
                "Ratio of frozen parameters vs. trainable parameters."
            )
        },
    )
    ...
    tie_breaker_strategy: str = field(
        ...
        metadata={
            "help": (
                "tie-breaker strategy for layer dropping: naive that drops the one in the front or activation-based that drops the one with max activation norm."
            )
        },
    )
...
def main():
    ...
    condense_epoch = data_args.condense_epoch
    max_budget = data_args.max_budget

    sparsity_ratio = float(data_args.sparsity_ratio)
    tie_breaker_strategy = data_args.tie_breaker_strategy
    # ------------------------------------- handle layerwise condensation related arguments ------------------------------------- #
    ...
    # --------------------------------- check state_dict for eval-only mode --------------------------------- #
    if training_args.do_eval and not training_args.do_train:
        state_dict = transformers.modeling_utils.load_state_dict(os.path.join(model_args.model_name_or_path, "pytorch_model.bin"))
    ...
    alpha_rate = 1.05
    peft_config = LoraConfig(task_type="text-generation", inference_mode=False, r=rank, lora_alpha=initial_alpha, lora_dropout=lora_dropout, target_modules=['q_proj', 'k_proj', 'v_proj', 'o_proj', 'gate_proj', 'up_proj', 'down_proj'])
    ...
    trainer = Condensation_Trainer(
        ...
        total_frozen_mask_str=model_args.total_frozen_mask_str,
        args=training_args,
    ...
    if training_args.do_eval:
        ...
        # this yields a tuple containing placeholders for ffn, attn removal list etc.
        metrics = trainer.evaluate()[-1]
